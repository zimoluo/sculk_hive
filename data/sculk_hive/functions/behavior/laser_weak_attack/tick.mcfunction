...
execute if score @s sh_boom_cd matches 55 as @e[tag=sh_main, limit=1, sort=nearest] at @s run playsound minecraft:entity.warden.sonic_charge hostile @a ~ ~ ~ 2

execute if score @s sh_boom_cd matches 45 anchored eyes run summon marker ^ ^ ^ {Tags:["sh_part", "sh_laser_weak","sh_laser_weak_raw"]}
execute as @e[tag=sh_laser_raw] at @s run function sculk_hive:behavior/laser_weak/init
