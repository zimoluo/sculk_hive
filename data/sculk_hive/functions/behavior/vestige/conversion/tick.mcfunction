execute if entity @s[type=skeleton] if data entity @s {StrayConversionTime: -1} if predicate sculk_hive:vestige/in_vestige if predicate sculk_hive:vestige/has_strength if data entity @s {OnGround: 1b} if block ~ ~-1 ~ sculk_catalyst run tag @s add sh_converting
execute if entity @s[type=zombie] if data entity @s {InWaterTime: -1, IsBaby: 0b, DrownedConversionTime: -1} if predicate sculk_hive:vestige/in_vestige if predicate sculk_hive:vestige/has_strength if data entity @s {OnGround: 1b} if block ~ ~-1 ~ sculk_catalyst run tag @s add sh_converting
...
execute unless block ~ ~-1 ~ sculk_catalyst run function sculk_hive:behavior/vestige/conversion/reset
