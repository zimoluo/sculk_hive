...
execute unless data storage sculk_hive:data {world:{difficulty: 0}} as @e[tag=sh_smn_special, tag=!sh_smn_special_ready] at @s positioned ~-9.5 ~-1 ~-9.5 if entity @a[gamemode=!spectator, dx=18, dy=3, dz=18] at @s unless block ~30 ~ ~30 void_air unless block ~-30 ~ ~30 void_air unless block ~30 ~ ~-30 void_air unless block ~-30 ~ ~-30 void_air run function sculk_hive:summon/structure_special/init
